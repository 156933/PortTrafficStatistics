...
// parseDateRange parses inclusive YYYY-MM-DD strings into a half-open [start, end) range
// where end has been advanced by one day. This makes "end" inclusive of the user-supplied
// date even though recorded_at columns hold full timestamps.
func parseDateRange(start, end string) (time.Time, time.Time, error) {
	...
	return startT, endT.AddDate(0, 0, 1), nil
}
...
func (s *Store) GetTrafficOverview(start, end string) ([]TrafficOverviewEntry, error) {
	startT, endT, err := parseDateRange(start, end)
	if err != nil {
		return nil, err
	}
	...
	rows, err := s.db.Query(query, startT, endT)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
