...
main_menu() {
    while true; do
        ...
        echo
        echo -e "  ${BOLD}管理${NC}"
        echo -e "  ${WHITE}3${NC}  更新 Server"
        echo -e "  ${WHITE}4${NC}  更新 Agent"
        echo -e "  ${WHITE}5${NC}  查看状态"
        echo -e "  ${WHITE}6${NC}  查看日志"
        echo -e "  ${WHITE}7${NC}  编辑配置"
        echo -e "  ${WHITE}8${NC}  卸载"
        echo
        echo -e "  ${WHITE}0${NC}  退出"
        line
        echo -ne "  请选择 ${DIM}[0-8]${NC}: "
        read -r choice

        case "$choice" in
            1) install_server ;;
            2) install_agent ;;
            3) update_binary server ;;
            4) update_binary agent ;;
            5) show_status ;;
            6) show_logs ;;
            7) edit_config ;;
            8) uninstall ;;
